...
while true:
    ...
    mask= cv2.inRange(imgSegFlipped,lb,up)
    ...
    final = maskClose
    _, const, h= cv2.findContours(maskClose, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    if(len(conts)!=0):
       b = max(conts,key=cv2.counterArea)
